...
const Hero = ({ handleOrderPopup }) => {
  ...
  setHeroSlides([
    {
      ...
      description: "Trendy collections available now.",
      buttonText: "Explore",
    ...
    },
    {
      _id: "2",
      title: "Style Up",
      description: "Level up your wardrobe today.",
      buttonText: "Shop Now",
      image: "/exclusive_image.png",
    },
  ]);
}, []);
  ...
  const fetchHeroSlides = async () => {
    try {
      ...
      console.log("Fetched data:", data); // Add this inside try block


      if (data.success) {
        setHeroSlides(data.heros);
      }
      console.log("heroSlides:", heroSlides); // Add after setHeroSlides
    } catch (error) {
        ...
        <Slider {...settings}>
          {heroSlides.map((data) => (
            <div key={data._id} className="outline-none">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center py-12 px-16">
                {/* Content Section */}
                <motion.div
                ...
                >
                  ...
                  <div className="flex gap-4">
                    <motion.button
                      ...
                      className="px-8 py-4 bg-pink-600  text-white rounded-xl font-semibold hover:bg-pink-700 transition-colors flex items-center gap-2"
                    >
                      {data.buttonText}
